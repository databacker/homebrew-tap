...
if [[ -z "${ASSETS_JSON}" ]] || ! echo "${ASSETS_JSON}" | grep -q '"assets":'
then
...
do
  ...
  if curl -L -f -o "${ASSET_PATH}" "${ASSET_URL}"
  then
    # Calculate SHA256
    ...
    if [[ -z "${EXPECTED_SHA256}" || "${EXPECTED_SHA256}" = "null" ]]
    then
    ...
    osucase="$(echo "${os}" | tr '[:lower:]' '[:upper:]')"
    archucase="$(echo "${arch}" | tr '[:lower:]' '[:upper:]')"
...
for var in $(set | awk -F= '/^SHA256_/ {print $1}')
do
  eval "sha256=\$${var}"
  # shellcheck disable=SC2154
  sed -i.tmp "s|${var}|${sha256}|g" "${FORMULA_FILE_WORKING}"
done
for var in $(set | awk -F= '/^ASSET_/ {print $1}')
do
  eval "asset=\$${var}"
  # shellcheck disable=SC2154
